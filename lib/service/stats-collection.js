...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'stor-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('stor').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('stor')[service]);
